...
function AdminPanel() {
  ...
  const [authed, setAuthed] = aS(
    () => sessionStorage.getItem("shame_admin") === "1",
  );
  const [pw, setPw] = aS("");
  ...
  const [disses, setDisses] = aS(() =>
    JSON.parse(JSON.stringify(window.DISSES || {})),
  );
  const [squad, setSquad] = aS(() => ({
    squad: window.SQUAD || [],
    stats: window.SQUAD_STATS || {},
  }));
  ...
  async function save(key, payload) {
    ...
    if (offline) {
      setStatus(
        "OFFLINE :: changes are local only :: deploy worker to persist",
      );
  ...
  function applyToWindow(key, payload) {
    ...
    if (key === "squad") {
      window.SQUAD = payload.squad;
      window.SQUAD_STATS = payload.stats;
    }
    if (key === "shame") {
    ...
    <div style={panelStyle}>
          ...
          {authed && (
            <span
              style={{
                ...
                fontSize: 10,
                color: offline ? "var(--hazard)" : "var(--acid)",
                letterSpacing: "0.2em",
              }}
            >
      ...
      {!authed ? (
        <div style={{ padding: "40px 24px", textAlign: "center" }}>
          ...
          />
          <button onClick={login} style={{ ...btnPrimary, marginLeft: 8 }}>
            AUTH
          </button>
          {status && (
            <div
          ...
          {offline && (
            ...
            >
              ⚠ NO WORKER CONFIGURED — any password unlocks local preview only.
              <br />
          ...
          <div style={footerStyle}>
            <span
              style={{
                fontFamily: "var(--font-mono)",
                fontSize: 11,
                color: dirty ? "var(--hazard)" : "var(--fg-2)",
                letterSpacing: "0.2em",
              }}
            >
              {dirty ? "● UNSAVED CHANGES" : "○ ALL SYNCED"}
              {status && (
                <span style={{ marginLeft: 16, color: "var(--cyan)" }}>
                  {status}
                </span>
              )}
            </span>
...
function DissesEditor({ data, onChange }) {
  ...
  function addLine() {
    ...
    if (section === "byPlayer") {
      ...
      next.squad = next.squad || [];
      next.squad.push("");
    } else if (section === "terminal") {
      next.terminal = next.terminal || [];
      next.terminal.push({ lvl: "INFO", msg: "" });
    } else if (section === "byHero") {
      next.byHero = next.byHero || {};
      next.byHero["New Hero"] = "";
    } else if (section === "byStat") {
      next.byStat = next.byStat || {};
      next.byStat["newStat"] = "";
    }
    onChange(next);
  }
  ...
  return (
    <div>
        ...
        {sections.map(([k, l]) => (
          <button
            ...
            style={{
              ...subTab,
              background:
                section === k ? "rgba(0,229,255,0.12)" : "transparent",
      ...
      {section === "byPlayer" &&
        Object.entries(data.byPlayer || {}).map(([pid, lines]) => (
        ...
        (data.terminal || []).map((entry, i) => (
          ...
          >
            <select
              ...
              onChange={(e) => {
                const next = JSON.parse(JSON.stringify(data));
                next.terminal[i].lvl = e.target.value;
                onChange(next);
              }}
              style={{ ...inputStyle, width: 80 }}
            >
              {["INFO", "WARN", "ERR", "CRIT"].map((l) => (
            ...
            <input
              ...
              onChange={(e) => {
                const next = JSON.parse(JSON.stringify(data));
                next.terminal[i].msg = e.target.value;
                onChange(next);
              }}
      ...
      {section === "byHero" &&
        Object.entries(data.byHero || {}).map(([hero, line], i) => (
          <div key={hero} style={{ display: "flex", gap: 6, marginBottom: 6 }}>
            <input
              value={hero}
              readOnly
              style={{ ...inputStyle, width: 160, opacity: 0.6 }}
            />
            <input
              value={line}
              onChange={(e) => {
                const next = JSON.parse(JSON.stringify(data));
                next.byHero[hero] = e.target.value;
                onChange(next);
              }}
              style={{ ...inputStyle, flex: 1 }}
            />
      ...
      {section === "byStat" &&
        Object.entries(data.byStat || {}).map(([k, v]) => (
    ...
    <div>
      ...
      <textarea
        ...
        style={{
          ...
          background: "#000",
          color: "var(--cyan)",
          border: "1px solid " + (err ? "var(--blood)" : "var(--border)"),
          fontFamily: "var(--font-mono)",
          fontSize: 12,
          padding: 12,
          lineHeight: 1.5,
